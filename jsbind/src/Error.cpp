...
Error::Error(Handle h) noexcept
    : emlite::Val(emlite::Val::take_ownership(h)) {}

Error Error::take_ownership(Handle h) noexcept {
    return Error(h);
}

Error::Error(const emlite::Val &val) noexcept
    : emlite::Val(val) {}
...
String Error::name() const {
    return get("name").template as<String>();
}
...
#define DEFINE_JS_ERROR(NAME)                              \
    NAME::NAME(Handle h) noexcept                          \
        : Error(emlite::Val::take_ownership(h)) {}         \
                                                           \
