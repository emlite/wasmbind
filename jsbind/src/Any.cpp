...
Any::Any(Handle h) noexcept
    : emlite::Val(emlite::Val::take_ownership(h)) {}

Any Any::take_ownership(Handle h) noexcept {
    return Any(h);
}
...
Any Any::null() noexcept {
    return Any{emlite::Val::null()};
}

[[nodiscard]] bool Any::isUndefined() const {
    return as_handle() == 1;
}
[[nodiscard]] bool Any::isNull() const {
    return as_handle() == 0;
}
