...
int64_t Date::value_of() const { return this->call("valueOf").template as<int64_t>(); }

/// Alias of valueOf()
int64_t Date::get_time() const { return value_of(); }

/// `toISOString()`
String Date::to_iso_string() const { return this->call("toISOString").template as<String>(); }

/// `toUTCString()`
String Date::to_utc_string() const { return this->call("toUTCString").template as<String>(); }

String Date::to_locale_string(const Any &locales, const Any &opts) const {
    if (!locales && !opts) {
...
Date Date::add_millis(int64_t delta) const { return from_epoch_millis(value_of() + delta); }

/// `other - this` (milliseconds)
int64_t Date::diff_millis(const Date &other) const { return other.value_of() - value_of(); }