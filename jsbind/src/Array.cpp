...
template <typename T>
size_t TypedArray<T>::push(const TypedArray<T> &items) noexcept {
    return this->call("push", items).template as<size_t>();
}

template <typename T>
TypedArray<T> TypedArray<T>::concat(const TypedArray<T> &items) noexcept {
    return this->call("concat", items).template as<TypedArray<T>>();
}

template <typename T>
String TypedArray<T>::join(const String &separator) noexcept {
...
template <typename T>
TypedArray<T> TypedArray<T>::sort(const Function &compareFn) noexcept {
...
template <typename T>
size_t TypedArray<T>::unshift(const TypedArray<T> &items) noexcept {
    return this->call("unshift", items).template as<size_t>();
}

template <typename T>
int TypedArray<T>::indexOf(const T &searchElement, size_t fromIndex) noexcept {
    return this->call("indexOf", searchElement, fromIndex).template as<int>();
}

template <typename T>
int TypedArray<T>::lastIndexOf(const T &searchElement, size_t fromIndex) noexcept {
    return this->call("lastIndexOf", searchElement, fromIndex).template as<int>();
}

template <typename T>
bool TypedArray<T>::every(const Function &predicate, const Any &thisArg) noexcept {
    return this->call("every", predicate, thisArg).template as<bool>();
}

template <typename T>
bool TypedArray<T>::some(const Function &predicate, const Any &thisArg) noexcept {
    return this->call("some", predicate, thisArg).template as<bool>();
}

template <typename T>
void TypedArray<T>::forEach(const Function &callbackfn, const Any &thisArg) noexcept {
    this->call("forEach", callbackfn, thisArg);
}

template <typename T>
TypedArray<Any> TypedArray<T>::map(const Function &callbackfn, const Any &thisArg) noexcept {
    return this->call("map", callbackfn, thisArg).template as<TypedArray<Any>>();
}

template <typename T>
TypedArray<T> TypedArray<T>::filter(const Function &predicate, const Any &thisArg) noexcept {
    return this->call("filter", predicate, thisArg).template as<TypedArray<T>>();
}

template <typename T>
Any TypedArray<T>::reduce(const Function &callbackfn, const Any &initialValue) noexcept {
    return this->call("reduce", callbackfn, initialValue).template as<Any>();
}

template <typename T>
Any TypedArray<T>::reduceRight(const Function &callbackfn, const Any &initialValue) noexcept {
    return this->call("reduceRight", callbackfn, initialValue).template as<Any>();
}
