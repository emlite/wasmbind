...
public:
    ...
    jsbind::Promise getDigitalGoodsService(const jsbind::DOMString& serviceProvider);
    DocumentPictureInPicture documentPictureInPicture() const;
    jsbind::Any event() const;
    Fence fence() const;
    jsbind::Promise showOpenFilePicker();
    jsbind::Promise showOpenFilePicker(const OpenFilePickerOptions& options);
    jsbind::Promise showSaveFilePicker();
    jsbind::Promise showSaveFilePicker(const SaveFilePickerOptions& options);
    jsbind::Promise showDirectoryPicker();
    jsbind::Promise showDirectoryPicker(const DirectoryPickerOptions& options);
    jsbind::Undefined captureEvents();
    jsbind::Undefined releaseEvents();
    External external() const;
    jsbind::Promise queryLocalFonts();
    jsbind::Promise queryLocalFonts(const QueryOptions& options);
    ...
    jsbind::Promise getScreenDetails();
